...
This adapter is an internal-network, read-only policy surface.  It is not an
execution runtime, approval engine, evidence store, memory engine or connector
gateway.  All policy meaning lives in :mod:`pantheon_mcp.service`.
"""
    ...
    *,
    ...
    async def enforce_body_limit(request: Request, call_next: Callable):
        if request.method in {"POST", "PUT", "PATCH"}:
            ...
            request._receive = receive  # Starlette request replay for downstream parsing.
    ...
    def health() -> dict[str, Any]:
        """Compatibility health projection; prefer /livez and /readyz."""
        return {
            "status": "ok",
            "mode": "read_only",
    ...
    @app.get("/v1/meta", dependencies=[Depends(require_api_key)])
    def meta() -> dict[str, Any]:
        return get_service().meta()

    @app.get("/v1/repository/state", dependencies=[Depends(require_api_key)])
    def repository_state() -> dict[str, Any]:
        return get_service().repository_state()

    @app.get("/v1/consultation", dependencies=[Depends(require_api_key)])
    ...
    @app.post(
        "/v1/observations/capabilities:qualify",
    ...
    @app.post(
        "/v1/candidates/task-contracts:prepare",
        dependencies=[Depends(require_api_key)],
    )
    ...
    @app.get("/runtime/context-pack", dependencies=[Depends(require_api_key)])
    ...
    @app.get("/domain/snapshot", dependencies=[Depends(require_api_key)])
