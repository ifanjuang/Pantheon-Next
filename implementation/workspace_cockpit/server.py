...
def _file_sha256(path: Path) -> tuple[str | None, str | None]:
    """Hash source bytes only when a cartouche explicitly asks for integrity verification."""
    try:
...
def _source_integrity(
    ...
    *,
    ...
    """Verify an explicitly declared source checksum without hashing every AFFAIRES file."""
    warnings: list[str] = []
