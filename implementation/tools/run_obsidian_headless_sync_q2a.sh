...
needle = sys.argv[2]
if needle not in json.dumps(payload):
...
# Safety sentinel for upstream concurrent-edit handling. Both edits must remain recoverable somewhere.
...
ob sync-status --path "$A" --json > "$ARTIFACTS/status-a.json"
ob sync-status --path "$B" --json > "$ARTIFACTS/status-b.json"
