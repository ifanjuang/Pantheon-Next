...
def create_app(
    *,
    ...
    root_value = document_root if document_root is not None else os.getenv("PANTHEON_DOCUMENT_ROOT")
    app.state.document_root = Path(root_value).resolve() if root_value else None
    app.state.api_key = api_key if api_key is not None else os.getenv("PANTHEON_COCKPIT_API_KEY", "")
    app.state.editor_api_key = (
        editor_api_key if editor_api_key is not None else os.getenv("PANTHEON_EDITOR_API_KEY", "")
    )
    app.state.hermes_api_key = (
        hermes_api_key if hermes_api_key is not None else os.getenv("PANTHEON_HERMES_API_KEY", "")
    )
    app.state.public_url = (
        public_url if public_url is not None else os.getenv("PANTHEON_COCKPIT_PUBLIC_URL", "")
    ).rstrip("/")
    ...
    app.state.policy_enforcement = (
        ...
        else os.getenv("PANTHEON_POLICY_ENFORCEMENT", "required")
    ).strip().lower()
    if app.state.policy_enforcement not in {"required", "disabled"}:
        raise ValueError(
            "PANTHEON_POLICY_ENFORCEMENT must be 'required' or 'disabled'; "
            f"got {app.state.policy_enforcement!r}"
        )
    if policy_client is not None:
        app.state.policy_client = policy_client
    else:
        policy_base_url = os.getenv("PANTHEON_POLICY_API_URL", "").strip()
        policy_api_key = os.getenv("PANTHEON_POLICY_API_KEY", "").strip()
        ...
        if client is None:
            raise HTTPException(
                ...
                detail=(
                    ...
                    "PANTHEON_POLICY_API_URL and PANTHEON_POLICY_API_KEY, or declare "
                    "PANTHEON_POLICY_ENFORCEMENT=disabled explicitly."
                ),
            )
        return client
        ...
        if (
            ...
            raise HTTPException(
                ...
                detail=(
                    ...
                    "admitted. Set PANTHEON_POLICY_API_URL and PANTHEON_POLICY_API_KEY, "
                    "or declare PANTHEON_POLICY_ENFORCEMENT=disabled explicitly."
                ),
            )
...
def run() -> None:
    ...
    uvicorn.run(
        "pantheon_app.cockpit_api:app",
        host=os.getenv("PANTHEON_COCKPIT_HOST", "127.0.0.1"),
        port=int(os.getenv("PANTHEON_COCKPIT_PORT", "8081")),
        reload=False,
    )
