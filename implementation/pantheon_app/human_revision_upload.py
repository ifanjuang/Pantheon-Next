...
class RevisionUploadConfig:
    ...
    def from_env_optional(cls) -> "RevisionUploadConfig | None":
        source_root = os.getenv("PANTHEON_HUMAN_SOURCE_ROOT", "").strip()
        retention_root = os.getenv("PANTHEON_RETENTION_ROOT", "").strip()
        provider_ref = os.getenv("PANTHEON_RETENTION_PROVIDER_REF", "").strip()
        configured = [bool(source_root), bool(retention_root), bool(provider_ref)]
        if not any(configured):
            return None
        if not all(configured):
            raise RevisionUploadConfigurationError(
                "PANTHEON_HUMAN_SOURCE_ROOT, PANTHEON_RETENTION_ROOT and PANTHEON_RETENTION_PROVIDER_REF must be configured together"
            )
        raw_limit = os.getenv("PANTHEON_HUMAN_UPLOAD_MAX_BYTES", str(DEFAULT_MAX_BYTES)).strip()
        try:
            max_bytes = int(raw_limit)
        except ValueError as exc:
            raise RevisionUploadConfigurationError(
                "PANTHEON_HUMAN_UPLOAD_MAX_BYTES must be an integer"
            ) from exc
        if max_bytes < 1:
            raise RevisionUploadConfigurationError(
                "PANTHEON_HUMAN_UPLOAD_MAX_BYTES must be positive"
            )
        return cls(
