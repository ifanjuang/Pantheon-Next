...
class OidcJwtVerifier:
    ...
    def from_env_optional(cls) -> "OidcJwtVerifier | None":
        issuer = os.getenv("PANTHEON_OIDC_ISSUER", "").strip()
        audience = os.getenv("PANTHEON_OIDC_AUDIENCE", "").strip()
        jwks_url = os.getenv("PANTHEON_OIDC_JWKS_URL", "").strip()
        algorithms_raw = os.getenv("PANTHEON_OIDC_ALGORITHMS", "RS256").strip()
        configured = [bool(issuer), bool(audience), bool(jwks_url)]
        if not any(configured):
            return None
        if not all(configured):
            raise AccessConfigurationError(
                "PANTHEON_OIDC_ISSUER, PANTHEON_OIDC_AUDIENCE and PANTHEON_OIDC_JWKS_URL must be configured together"
            )
