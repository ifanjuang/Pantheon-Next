...
def main() -> int:
    parser = argparse.ArgumentParser(prog="pantheon-app")
    ...
    if args.command == "bind-oidc-identity":
        ...
        enforcement = (os.getenv("PANTHEON_POLICY_ENFORCEMENT", "required") or "").strip()
        if enforcement not in {"required", "disabled"}:
            ...
            return 1
        base_url = os.getenv("PANTHEON_POLICY_API_URL", "").strip()
        api_key = os.getenv("PANTHEON_POLICY_API_KEY", "").strip()
        policy_client = (
        ...
        if enforcement == "required" and policy_client is None:
            print(
                ...
                "Set PANTHEON_POLICY_API_URL and PANTHEON_POLICY_API_KEY, or declare "
    ...
    if args.command == "store-reviewed-dossier":
        ...
        if enforcement == "required" and policy_client is None:
            print(
                ...
                "reviewed APU dossier cannot be admitted. Set PANTHEON_POLICY_API_URL "
                "and PANTHEON_POLICY_API_KEY, or declare "
                "PANTHEON_POLICY_ENFORCEMENT=disabled explicitly.",
                file=sys.stderr,
            )
            return 1
