...
# human-signer discipline (pantheon_app/signer.py); the register seam reuses them.
...
def allowed_decisions() -> frozenset:
    return frozenset(
        pantheon_contracts.definition_enum(
            "governed_loop_objects", "decision_value"
        )
    )
...
def _assert_conforms(obj: dict, what: str) -> None:
    ...
    try:
        pantheon_contracts.validate("governed_loop_objects", obj)
...
def _validate_record(record: dict) -> None:
    ...
    try:
        pantheon_contracts.validate("governed_loop_objects", record)
    except pantheon_contracts.ContractViolation as exc:
        raise GateRefusal(
