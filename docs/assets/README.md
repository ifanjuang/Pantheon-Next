# Assets Registry

Status: active support index — documentation assets only.

This directory contains explanatory and governance-grammar assets. It is not runtime, an approval surface, a memory engine or a source of professional truth.

```text
OpenWebUI exposes.
Hermes Agent executes.
Pantheon Next governs.
The human decides.
```

## Current assets

| Asset | Purpose | Status |
|---|---|---|
| [`pantheon-map/`](pantheon-map/) | Ecosystem map for exposure, governance, execution, evidence and Register boundaries. | Documentation only. |
| [`workflow-under-hood/`](workflow-under-hood/) | Explanatory professional workflow and evidence-gap path. | Documentation only. |
| [`pantheon-control/`](pantheon-control/) | Stable orientation page toward the external MVP cockpit. Contains only `README.md` and `index.html`. | Pointer only; no local dashboard. |
| [`card-stack/`](card-stack/) | Governance-grammar prototypes derived from `CARD_STACK_MODEL.md`. | Illustrative; documented non-implemented. |
| [External MVP cockpit demo](https://ifanjuang.github.io/pantheon-mvp/) | Synthetic no-network demonstration using the `pantheon-mvp` cockpit assets. Source observed at [`7f3faf74afd59a07a9ab6026360881eb374df905`](https://github.com/ifanjuang/pantheon-mvp/tree/7f3faf74afd59a07a9ab6026360881eb374df905). | Implemented externally; public availability, adoption and activation remain separate. |
| [`pantheon-flow/entrees-sorties-memoire-d3.html`](pantheon-flow/entrees-sorties-memoire-d3.html) | Explanatory flow for context, candidates, human decision and governed records. | Documentation only. |
| [`pantheon-rpg/`](pantheon-rpg/) | Narrative illustrations for responsibility paths. | Documentation only. |

## Placement rule

Visual material stays in Pantheon Next when it explains governance, conformance or a non-executable interaction grammar.

Product screens, executable renderers, synthetic project data and runtime scenarios belong in `ifanjuang/pantheon-mvp`.

Superseded mockups are removed rather than migrated. Git history and `ai_logs/` preserve the trace.

See `../governance/NEXT_MVP_REPOSITORY_PLACEMENT.md` for the full boundary.

## Status rule

Every asset must visibly distinguish:

```text
implemented
implemented externally
partial / to verify
documented non-implemented
candidate
obsolete
voluntarily absent
```

A visible button or workflow must not imply execution unless the implementation and runtime binding are separately established.

## Public wording

The retired `pantheon-control/` path must be described as an orientation or external binding pointer, not as a Pantheon Next dashboard.

Preferred labels:

```text
External MVP cockpit demo
Orientation vers le cockpit MVP
Governance-grammar prototype
```

Avoid unqualified claims such as `dashboard live`, `services en ligne`, `connexions actives` or `control plane opérationnel` without current evidence.

## Evidence and memory wording

```text
Hermes memory     -> runtime recall without authority
Knowledge         -> organized reference, not automatically true or approved
Evidence          -> scoped support deliberately admitted for a claim
Registre          -> governed durable record after required validation
```

Display does not create probative status.

## Anti-runtime reminder

This directory contains no runtime resources, deployment configuration, approval engine, scheduler, queue, provider router, installer, memory promotion engine or external-action component.
