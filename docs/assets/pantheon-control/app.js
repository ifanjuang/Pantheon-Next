/* Pantheon Control — hierarchy-driven deck app.
   ...
   - bas = descendre, haut = remonter ;
   - Swipers horizontaux imbriqués = cartes sœurs ;
   - gauche / droite = changer de sibling au même niveau ;
...
(function(){
  ...
  const CARD_TYPES = {
    ...
    method:{label:'Méthode', className:'pc-card--method', bg:'METHOD', fields:['family','authority','trigger','next']},
  ...
  const CONFIG = window.PC_DECK_CONFIG || {
    root:{id:'root', type:'root', title:'Pantheon Control', scene:'runs', children:[
      {id:'project-general', type:'project', title:'Général / Corpus IFJ', scene:'knowledge', summary:'Corpus hors projet : connaissances, méthodes, guides, lexiques, gates types et runs types.', phase:'Global', scope:'Hors projet', next:'Réutiliser comme référence, jamais comme preuve projet sans Evidence scopée.', chips:[['Corpus','blue'],['Non probant seul','yellow'],['Hors projet','muted']], children:[
        ...
        {id:'general-methods', type:'scene', title:'Méthodes', scene:'runs', summary:'Cartes Méthodes globales : elles structurent le travail, elles ne prouvent ni n’exécutent.', description:'Deck neutre : méthodes professionnelles visibles, modes de raisonnement bruts et patterns runtime séparés.', next:'Mobiliser dans une tâche seulement si un rôle détecte une tension.', chips:[['Méthodes','blue'],['Non runtime','muted']], children:[
          {id:'general-methods-professional', type:'subject', title:'Méthodes professionnelles', scene:'runs', summary:'Méthodes affichables dans le cockpit métier.', scope:'Global · Méthodes', next:'Référencer dans un run sans les rendre project-owned.', chips:[['Deck métier','blue']], children:[
            {id:'method-source-admission', type:'method', title:'source_admission', scene:'runs', summary:'Qualifier une source avant qu’elle soutienne une assertion.', family:'professional_method', authority:'Candidate support doctrine', trigger:'Nouveau document, mail, photo, devis ou formulaire.', next:'Retourner source candidate + questions ; jamais preuve finale.', chips:[['Source','yellow'],['ARGOS','blue']]},
            {id:'method-mission-scope', type:'method', title:'mission_scope_guard', scene:'runs', summary:'Détecter une formulation ou action qui dépasse la mission.', family:'professional_method', authority:'Candidate support doctrine', trigger:'Validation, instruction, visa, responsabilité, paiement ou assurance.', next:'Proposer reformulation prudente ou gate.', chips:[['THEMIS','orange'],['Gate possible','red']]},
            {id:'method-probative-review', type:'method', title:'probative_review', scene:'runs', summary:'Vérifier le support probatoire d’une sortie candidate.', family:'professional_method', authority:'Candidate support doctrine', trigger:'Une sortie peut justifier décision, mémoire ou transmission.', next:'Retourner gaps, contradictions, confiance et questions.', chips:[['Preuve','yellow'],['ARGOS','blue']]}
          ]},
          {id:'general-methods-runtime', type:'subject', title:'Patterns runtime', scene:'runs', summary:'Patterns LLM/Hermes qui peuvent aider mais ne deviennent pas autorité professionnelle.', scope:'Global · Runtime patterns', next:'Utiliser côté Hermes sous Task Contract, pas comme carte d’autorité.', chips:[['Hermes-side','muted']], children:[
            {id:'method-runtime-cove', type:'method', title:'chain_of_verification', scene:'runs', summary:'Pattern de vérification interne, exposé seulement comme justification contrôlable.', family:'runtime_pattern', authority:'Candidate guide de compétence', trigger:'Preuve faible ou assertions nombreuses.', next:'Ne jamais remplacer Evidence Pack.', chips:[['Runtime','muted']]}
          ]}
        ]},
      ...
      {id:'project-les-damps', type:'project', title:'Les Damps', scene:'runs', summary:'Projet actif : suivi chantier, CR, factures, documents et gates.', phase:'DET / chantier', scope:'Extension + rénovation énergétique', next:'Contrôler les gates ouverts avant transmission.', chips:[['Actif','green'],['Gates 4','red']], children:[
        {id:'damps-runs', type:'scene', title:'Runs', scene:'runs', summary:'Sessions de traitement et brouillons candidats.', description:'CR, factures, façade et production de sorties candidates.', next:'Ouvrir un sujet.', chips:[['Alertes 3','orange']], children:[
          {id:'damps-cr-runs', type:'subject', title:'CR chantier', scene:'runs', summary:'Comptes rendus chantier.', scope:'Projet Les Damps · Runs', next:'Valider le draft avant envoi.', chips:[['En cours','blue']], children:[
            {id:'run-cr-2026-06-27', type:'run', title:'CR chantier 27/06', scene:'runs', summary:'Brouillon CR prêt, 3 gates ouverts, 2 points à vérifier.', date:'2026-06-27', indice:'A', output:'Final Candidate v1.0 non transmis.', next:'Relire et décider.', chips:[['Candidate','blue'],['Gates 3','red']], children:[
              {id:'method-cr-source-admission', type:'method', title:'source_admission', scene:'runs', summary:'Méthode mobilisée pour qualifier notes, ancien CR et photos avant rédaction.', family:'professional_method', authority:'Task-level reference', trigger:'Sources hétérogènes du CR.', next:'Qualifier avant assertion.', chips:[['Méthode','blue']]},
              {id:'method-cr-mission-scope', type:'method', title:'mission_scope_guard', scene:'runs', summary:'Sous-carte de tâche : formulations engageantes à contrôler avant envoi.', family:'professional_method', authority:'Method Proposal Candidate', trigger:'Draft CR destiné à une transmission externe.', next:'Ouvrir gate si formulation engage.', chips:[['THEMIS','orange'],['Gate','red']]},
              {id:'gate-cr-method', type:'gate', title:'Gate envoi après méthodes', scene:'gates', summary:'Les méthodes structurent la revue ; seul le gate autorise la transmission.', decision:'Envoyer / corriger / garder brouillon.', reason:'Effet externe.', next:'Décision humaine explicite.', chips:[['Ouvert','red']]}
            ]},
          ...
          {id:'damps-factures-runs', type:'subject', title:'Factures', scene:'runs', summary:'Factures, situations et devis supplémentaires.', scope:'Projet Les Damps · Runs', next:'Croiser marché, CCTP et avancement.', chips:[['Paiement','red']], children:[
            {id:'run-facture-msb-01', type:'run', title:'Analyse facture MSB', scene:'runs', summary:'Comparer facture, AE, CCTP et avancement.', date:'2026-06-28', indice:'A', output:'Avis candidat : paiement à vérifier.', next:'Gate paiement requis.', chips:[['Gate paiement','red']], children:[
              {id:'method-facture-contract', type:'method', title:'contractual_decomposition', scene:'runs', summary:'Découper lot, clause, prestation et montant avant avis.', family:'professional_method', authority:'Task-level reference', trigger:'Facture / situation à comparer au marché.', next:'Retourner analyse candidate + gaps.', chips:[['THEMIS','orange']]},
              {id:'method-facture-quote', type:'method', title:'quote_variation_review', scene:'runs', summary:'Vérifier plus-value, moins-value ou devis complémentaire sans approbation silencieuse.', family:'professional_method', authority:'Method Proposal Candidate', trigger:'Écart entre facture, devis ou CCTP.', next:'Ne pas approuver ; préparer gate.', chips:[['ARGOS','blue'],['Paiement','red']]}
            ]}
          ]},
  ...
  function initSwipers(){
    ...
    document.querySelectorAll('.pc-level-swiper').forEach(el => {
      ...
      const swiper = new Swiper(el, {direction:'horizontal', slidesPerView:1, spaceBetween:14, initialSlide:state.path[level] || 0, keyboard:false, pagination:{el:el.querySelector('.pc-level-pagination'), clickable:true}, resistanceRatio:.72, nested:true, mousewheel:false});
    ...
    if(!depthEl) return;
    state.depthSwiper = new Swiper(depthEl, {direction:'vertical', slidesPerView:1, spaceBetween:0, initialSlide:state.activeLevel || 0, keyboard:{enabled:true, onlyInViewport:true}, pagination:{el:depthEl.querySelector('.pc-depth-pagination'), clickable:true}, resistanceRatio:.72, nested:false});
