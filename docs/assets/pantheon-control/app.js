...
(function(){
  ...
  const CONFIG = window.PC_DECK_CONFIG || {
    root:{id:'root', type:'root', title:'Pantheon Control', scene:'runs', children:[
      ...
      {id:'project-les-damps', type:'project', title:'Les Damps', scene:'runs', summary:'Projet actif : suivi chantier, CR, factures, documents et gates.', phase:'DET / chantier', scope:'Extension + rénovation énergétique', next:'Contrôler les gates ouverts avant transmission.', chips:[['Actif','green'],['Risque moyen','yellow'],['Gates 4','red']], children:[
  ...
  function breadcrumb(level){
    ...
    for(let i=0;i<=level;i++){
      const node = selectedNodeAtLevel(i);
      if(node) parts.push({title:node.title, level:i});
    }
    return parts;
  }
