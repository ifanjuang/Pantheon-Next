...
(function(){
      ...
      {id:'project-general', type:'project', title:'Général / Corpus IFJ', scene:'knowledge', summary:'Corpus hors projet : connaissances, guides, lexiques, gates types et runs types.', phase:'Global', scope:'Hors projet', next:'Réutiliser comme référence, jamais comme preuve projet sans Evidence scopée.', chips:[['Corpus','blue'],['Non probant seul','yellow'],['Hors projet','muted']], children:[
        ...
        {id:'general-runs', type:'scene', title:'Runs types', scene:'runs', summary:'Méthodes de traitement réutilisables sans effet d’exécution.', description:'Scénarios types pour CR, factures, photos chantier et matériaux.', next:'Adapter à un projet réel.', chips:[['Non exécuté','muted']], children:[
          {id:'general-run-cr', type:'subject', title:'CR chantier', scene:'runs', summary:'Finaliser un CR depuis notes, photos, ancien CR et contraintes.', scope:'Méthode type', next:'Tester sur dossier réel.', chips:[['Run type','blue']], children:[
            {id:'run-type-cr-final', type:'run', title:'Run type — finalisation CR', scene:'runs', summary:'Lire notes, comparer ancien CR, produire draft, ouvrir gates.', output:'Draft CR + mail candidat + gates.', next:'Ne pas envoyer sans gate.', chips:[['Gate externe','red']]},
            {id:'run-type-photo-doute', type:'run', title:'Run type — photo doute chantier', scene:'runs', summary:'Analyser photo, rechercher contexte, demander autre angle si insuffisant.', output:'Observation candidate ou gap ciblé.', next:'Ne pas conclure sans source.', chips:[['Gap possible','orange']]}
          ]},
          {id:'general-run-facture', type:'subject', title:'Facture / devis', scene:'runs', summary:'Analyser facture, situation ou devis supplémentaire.', scope:'Méthode type', next:'Croiser marché, CCTP, assurance et avancement.', chips:[['Paiement','red']], children:[
            {id:'run-type-facture', type:'run', title:'Run type — analyse facture', scene:'runs', summary:'Comparer facture, marché, CCTP, avancement et mission.', output:'Avis candidat + points à vérifier + gate paiement.', next:'Décision humaine obligatoire.', chips:[['Gate paiement','red']]}
          ]}
        ]},
      ...
      {id:'project-les-damps', type:'project', title:'Les Damps', scene:'runs', summary:'Projet actif : suivi chantier, CR, factures, documents et gates.', phase:'DET / chantier', scope:'Extension + rénovation énergétique', next:'Contrôler les gates ouverts avant transmission.', chips:[['Actif','green'],['Gates 4','red']], children:[
        {id:'damps-runs', type:'scene', title:'Runs', scene:'runs', summary:'Sessions de traitement et brouillons candidats.', description:'CR, factures, façade et production de sorties candidates.', next:'Ouvrir un sujet.', chips:[['Alertes 3','orange']], children:[
          {id:'damps-cr-runs', type:'subject', title:'CR chantier', scene:'runs', summary:'Comptes rendus chantier.', scope:'Projet Les Damps · Runs', next:'Valider le draft avant envoi.', chips:[['En cours','blue']], children:[
            {id:'run-cr-2026-06-27', type:'run', title:'CR chantier — 27/06', scene:'runs', summary:'Brouillon CR prêt, 3 gates ouverts, 2 points à vérifier.', output:'Final Candidate v1.0 non transmis.', next:'Relire et décider.', chips:[['Candidate','blue'],['Gates 3','red']]},
            {id:'draft-cr-v1', type:'draft', title:'Draft CR — v1.0', scene:'runs', summary:'Compte rendu candidat non envoyé.', version:'v1.0', output:'CR + mail candidat.', next:'Vérifier formulations hors mission.', chips:[['Non envoyé','red']]}
          ]},
          {id:'damps-factures-runs', type:'subject', title:'Factures', scene:'runs', summary:'Factures, situations et devis supplémentaires.', scope:'Projet Les Damps · Runs', next:'Croiser marché, CCTP et avancement.', chips:[['Paiement','red']], children:[
            {id:'run-facture-msb-01', type:'run', title:'Analyse facture MSB — situation 01', scene:'runs', summary:'Comparer facture, AE, CCTP et avancement.', output:'Avis candidat : paiement à vérifier.', next:'Gate paiement requis.', chips:[['Gate paiement','red']]}
          ]},
          {id:'damps-facade-runs', type:'subject', title:'Façade', scene:'runs', summary:'Choix matériaux, PLU, échanges instruction et économie.', scope:'Projet Les Damps · Runs', next:'Croiser PLU et budget.', chips:[['PLU requis','yellow']], children:[
            {id:'run-facade-materials', type:'run', title:'Choix façade — matériaux', scene:'runs', summary:'Comparer bardage, enduit, brique, coût et acceptabilité PLU.', output:'Pré-sélection candidate.', next:'Vérifier PLU et échanges antérieurs.', chips:[['Candidate','blue']]}
          ]}
        ]},
  ...
  function renderCardBreadcrumb(level, index){
    return '<nav class="pc-card-breadcrumb">'+crumbsForPath(pathForCard(level, index)).map((crumb, i) => '<button data-card-crumb="'+esc(crumb.path.join(','))+'">'+esc(crumb.title)+'</button>').join('<span>/</span>')+'</nav>';
  }
  ...
  function labelFor(key){ return ({phase:'Phase', scope:'Périmètre', next:'Prochaine action', description:'Description', output:'Sortie', source:'Source', freshness:'Fraîcheur', event:'Événement', impact:'Impact', open:'Ouvert', blocked:'Bloqué', decision:'Décision', reason:'Motif', family:'Famille', authority:'Autorité', version:'Version'})[key] || key; }
  ...
  function renderCard(node, level, index){
    ...
    return '<article class="pc-card '+esc(cfg.className)+sceneClass+'" data-card-level="'+level+'" data-card-index="'+index+'" data-node-id="'+esc(node.id || '')+'">'+
      breadcrumb+
