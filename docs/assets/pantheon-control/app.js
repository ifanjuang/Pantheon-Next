...
(function(){
  ...
  const PC_DECK_CONFIG = window.PC_DECK_CONFIG || {
    ...
    root:{
      id:'root', type:'root', title:'Pantheon Control', scene:'runs', children:[
        {
          ...
          children:[
            {
              ...
              summary:'Sources documentaires générales avant qualification.',
              description:'PLU, MAF, guides CCTP, lexiques et doctrine agence.', next:'Qualifier autorité, version et fraîcheur.',
              chips:[['Scène','yellow'],['Sources','muted']], children:[
            ...
            },
            {
              ...
              description:'Scénarios types pour CR, factures, photos chantier et matériaux.', next:'Adapter à un projet réel.',
              chips:[['Scène','blue'],['Non exécuté','muted']], children:[
  ...
  function renderCard(node, index){
    ...
    return '<article class="pc-card '+esc(cfg.className)+sceneClass+'" data-index="'+index+'" data-node-id="'+esc(node.id || '')+'"><div class="pc-card__bg-word">'+esc(node.bg || cfg.bg || cfg.label)+'</div><div class="pc-card__inner"><header class="pc-card__header"><span class="pc-card__eyebrow">'+esc(cfg.label)+' · '+esc(node.type || 'carte')+'</span><h3 class="pc-card__title">'+esc(node.title)+'</h3></header><p class="pc-card__summary">'+esc(node.summary || '')+'</p>'+(fields ? '<div class="pc-card__fields">'+fields+'</div>' : '')+'<footer class="pc-card__footer">'+chips+action+'</footer></div></article>';
  }
