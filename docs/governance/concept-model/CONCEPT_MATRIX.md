...
| Evidence | `EVIDENCE_PACK.md`, `EVIDENCE_TOPOLOGY.md` and applicable Register doctrines | Evidence Card, Evidence Scene, Decision Surface | evidence producers may run externally; acceptance is not runtime-owned | evidence acceptance and consequential reliance | doctrine present; prototype projection present; runtime success remains non-evidence |
| Gate | `APPROVALS.md` and applicable gate-specific doctrines | Gate Card, Decision Surface, Work Scene | no autonomous execution seat; enforcement may be implemented by bounded policy checks | opening, satisfying, refusing or escalating a consequential gate | doctrine present; prototype projection present; no automatic approval engine |
...
| Register | `MEMORY.md`, `EVIDENCE_MEMORY_CANONICALIZATION.md` and Register owner contracts | Register links, Evidence/Trace views and future governed record projections | external storage may persist only through an authorized seam; Pantheon governs admission | durable retention, supersession and admission | doctrine and some declarative contracts exist; no automatic memory-promotion engine |
...
| Card | `CARD_STACK_MODEL.md` | Card Stack prototypes and future OpenWebUI exposure | none | none by the Card itself; interactions may route to review | projection grammar documented; static prototype present; no production renderer |
| Scene | `CARD_STACK_MODEL.md` | Work, Evidence, Assets, Decisions and Trace scenes | none | scene composition must not hide required review information | bounded projection grammar documented; static mobile prototype present; no Scene engine |
