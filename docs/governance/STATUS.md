...
`AUTHORITY_INDEX.md` defines the repository authority classes and status vocabulary for distinguishing canonical doctrine, active support doctrine, candidates, validation-only material, external references, implementation artifacts, voluntarily absent elements and obsolete or refused material.

It does not promote candidates by itself.

It does not implement runtime behavior, schemas, tests, operations, platform code, Docker, environment configuration, approval engines or memory engines.
...
Do not call documented doctrine implemented runtime.
```
