...
| iFixAi | agent assurance evaluation: adversarial inspections, structural governance checks, LLM judging and run manifests | future bounded Hermes evaluation runner against a synthetic sandbox only | to review / boundary required | `WATCHLIST.md`, `DISTILLATION_REGISTRY.md`, future Hermes binding review only after sandbox evidence | measurable-coverage disclosure, declared-vs-observed separation, conservative insufficient-evidence handling and reproducible run metadata | declared or synthesized governance scored as if runtime-observed; score mistaken for certification, proof, approval or production gate; scorecards/checkpoints retain full model I/O; multi-provider disclosure and telemetry |
