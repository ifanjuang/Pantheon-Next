...
## Capability candidate distillations (2026-07-07)

These rows replace the transient one-shot review files from PR #296. They preserve the capability posture without keeping long reference reviews as doctrine.

| Tool / repo | Abstract capability | Hermes binding candidate | Status | Target doctrine document | Safe default | Main risk |
|---|---|---|---|---|---|---|
| Chunky (`GiovanniPasq/chunky`) | prepare, inspect and chunk documents before RAG | `chunky-local-docker` | `accepted_for_sandbox` | `CAPABILITY_CANDIDATE_MODEL.md`, `RAG_INGESTION_AND_EVIDENCE_BOUNDARIES.md` | non-sensitive PDFs only; local execution; no Cloud API; no indexation without human approval | chunks or Markdown candidates treated as evidence, truth or memory |
| Meetily | meeting capture / transcription / summary candidate | no binding approved | `quarantined_capability_candidate` | `CAPABILITY_CANDIDATE_MODEL.md`, `EXTERNAL_TOOLS_POLICY.md` | reference review only; no agency install; no client meeting use; analytics/updater/external providers blocked pending review | meeting data, consent, analytics or provider calls escaping governed scope |
| SurgicalFS MCP Server | local filesystem access candidate for adapter design | `surgicalfs-local-readonly` | `accepted_for_adapter_design` | `CAPABILITY_CANDIDATE_MODEL.md`, `HERMES_CODE_HOSTING_BOUNDARY.md` | local stdio; read-only; disposable test directory; no client data; no repo write; no HTTP/tunnel/dashboard exposure | filesystem capability escalating into write/runtime/MCP-host authority |
