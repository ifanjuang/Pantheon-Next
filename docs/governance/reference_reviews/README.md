...
| Future AGI | AI reliability suite: eval, simulation, guardrails, tracing, optimization and gateway | optional bounded pre-execution simulation / evaluation-lab adapter only | distilled; current upstream delta reviewed 2026-07-16 | `PRE_EXECUTION_SIMULATION.md`, `HERMES_CAPABILITY_BINDINGS.md`, `EXTERNAL_TOOL_PLACEMENT_REGISTER.md` | simulation-before-high-risk-task and trajectory-evaluation patterns | full platform duplicates Hermes; privileged executor; telemetry and professional-data leakage; optimizer or score becoming authority |
| BlitzOS | versioned multi-repository context, warm-start session handoff and Claude Cloud launch portal | Pantheon-native Context Pack / handoff adapter for Hermes; no BlitzOS runtime dependency | distilled 2026-07-16 | `CONTEXT_PACKS.md`, `DISTILLATION_REGISTRY.md`, `HERMES_CAPABILITY_BINDINGS.md`, `EXTERNAL_TOOL_PLACEMENT_REGISTER.md` | portable repo map, concise factual handoff and candidate-only context maintenance | context repo treated as canonical memory; direct default-branch log writes; automatic skill copying; broad OAuth and Claude-specific runtime coupling |
