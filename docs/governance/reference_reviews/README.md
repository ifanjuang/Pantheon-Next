...
| LangGraph | graph-structured agent runtime | optional runtime candidate under Task Contract | to review | `EXTERNAL_TOOLS_POLICY.md` | boundary stress-test vocabulary (drift taxonomy) | central-runtime import; highest runtime-drift risk reviewed |
...
| OpenTakeoff (`Kentucky-ai/opentakeoff`) | local drawing/quantity takeoff with shared human/agent geometry, provenance, deterministic correction rules and scored corpus | optional `drawing_takeoff_local` binding behind the APU Task Contract; exact pinned package only after separate adoption review | distilled / evaluation and adoption remain separate | `docs/domain-packs/architecture/DRAWING_TAKEOFF_LOCAL_ADAPTER.md` | independent PDF quantity/source witness plus strong parity, refusal, provenance and benchmark patterns; subordinate Revit engineering-conformance note applies those patterns to the plugin reference skeleton | engine session, quantity, review flag or upstream MCP registry mistaken for Pantheon truth/adoption; US-unit assumptions leaking into project contracts |
