...
| `docs/HTTP_API_CONTRACT.md` | implementation support pointer / public navigation | implemented pointer / not authority | Stable repository documentation pointer to the protected HTTP adapter contract. It contains no independent policy logic and must not diverge from the package contract. |
| `Dockerfile.policy-api` | infrastructure / deployment candidate | implemented candidate / protected path / not activated | Hardened candidate image for `pantheon-policy-api`, running as a non-root user and installing only the bounded package plus HTTP extra. File presence is not installation, health, activation, approval or production authorization. |
| `compose.policy-api.yaml` | infrastructure / deployment candidate | implemented candidate / protected path / not activated | Candidate internal `ai-net` deployment with no host port, read-only checkout/filesystem, dropped Linux capabilities, no Docker socket and application-level bearer authentication. File presence is not installation, health, activation, approval or production authorization. |
...
| `Docker*` | infrastructure / runtime artifact | protected path | Do not modify without explicit confirmation. A reviewed candidate file is not an activated deployment. |
| `compose*.yaml` | infrastructure / runtime artifact | protected path | Do not modify without explicit confirmation. A reviewed candidate file is not an activated deployment. |
