...
| Hermes Modules dashboard plugin | implementation artifact / external adapter | implemented in repository / host installation to verify | Bounded dashboard plugin intended to run inside Hermes. It may read declared Hermes/runtime state and expose governed classifications, including confirmed controls for bounded external jobs. Hermes hosts and executes it. Repository presence does not prove installation, reachability, health, activation, safety or approval on a particular host. |
| Shared Pantheon Control renderer | implementation artifact / presentation layer | implemented / bounded | Shared renderer used by the Hermes dashboard binding and the static demo projection. It renders supplied status data; it does not discover runtimes by itself, execute actions, approve, install, update, schedule, route providers or promote memory. |
| `docs/assets/pantheon-control/` | implementation artifact / static demonstration bundle | implemented static synthetic demo / partial projection | GitHub Pages-compatible bundle using synthetic fixtures and the shared renderer. It demonstrates `DEMO`, `LIVE` and `LIVE_PARTIAL` presentation semantics but is not evidence that a live Hermes host is connected. It is not a live cockpit, approval engine, memory engine, runtime, sender, scheduler or provider router. |
