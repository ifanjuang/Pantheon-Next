...
| `HERMES_CODE_HOSTING_BOUNDARY.md` (removed; Git history) | obsolete hosting decision | superseded by current monorepo placement and Hermes integration boundary | The July 2026 Option A decision placed executable candidate code in a sibling repository. Current placement is owned by `NEXT_MVP_REPOSITORY_PLACEMENT.md`: bounded Pantheon implementation/adapters live under `implementation/`, while Hermes remains the external execution runtime under `HERMES_INTEGRATION.md`. The historical arbitration remains in Git and dated `ai_logs`. |
