...
| `docs/governance/REGISTER_LINK_CASCADE_SCHEMA_PROPOSAL.md` | validation-only | documented non-implemented | Proposal for two candidate schemas — `register_link` (typed relations between register entries) and `impact_review` (cascade) — formalizing the dependency/impact model already in `EVIDENCE_MEMORY_CANONICALIZATION.md`. Declarative contract; no engine, no auto-resolution. Touches no protected path; apply blocked pending approval. |
