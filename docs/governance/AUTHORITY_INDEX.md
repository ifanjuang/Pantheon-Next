...
| `docs/governance/ARCHITECTURE_PROJECT_DATA_REGISTER.md` | candidate support doctrine | documented non-implemented | Candidate discipline for architecture project identity, aliases, sources, facts, derived regulatory candidates, decisions, evidence and transmissions. Reconcile with the architecture domain pack and data-platform cluster; adds no schema, runtime, connector, approval or memory engine. |
