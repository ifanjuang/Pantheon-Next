...
| `docs/governance/AXIS_VOCABULARY_RECONCILIATION_PROPOSAL.md` | validation-only | documented non-implemented | Coordinated schema + example proposal to align residual `confidence` (→ `certainty`) and `approval_impact` (→ `approval_ceiling`) deviations with the E/V/K/C axes. Declarative; applied only after approval. |
