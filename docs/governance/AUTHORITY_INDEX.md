...
| `docs/domain-packs/architecture/` | candidate support doctrine (grouped) | to verify | Architecture domain pack (grouped row). Individual rows migrated to `docs/governance/authority/ARCHITECTURE_AUTHORITY_INDEX.md`; this grouped row keeps the area visible and covers members without an individual row. |
...
| `docs/governance/authority/` | candidate support maps | partial | Sub-indexes per `AUTHORITY_INDEX_DECOMPOSITION_PLAN.md`. The obsolete/absent, architecture and external-references maps are populated; the others are skeletons. They list placement only and must not override this index's vocabulary. See the Sub-index map section below. |
...
Sub-indexes decompose the Current authority map by area, per `AUTHORITY_INDEX_DECOMPOSITION_PLAN.md`. Migration proceeds group by group through reviewed PRs: the obsolete/absent map is populated (obsolete documents, voluntary absences and the historical bootstrap stub row live there), the architecture map carries the `docs/domain-packs/architecture/` rows, and the external-references map carries the distillation and external-register rows. The remaining sub-indexes are skeletons, and for their scopes the Current authority map above remains the complete, authoritative map. Candidate documents under `docs/governance/` stay indexed here regardless of scope until the coverage checker is extended in a separately approved PR, because `.github/scripts/check_index_coverage.py` reads only this file. A sub-index may only list where documents sit; it must not override the authority vocabulary, promotion rule, placement test, tool naming rule, terminology boundary rule or sensitive-path guardrail defined here.
...
| Architecture domain | `docs/governance/authority/ARCHITECTURE_AUTHORITY_INDEX.md` | candidate support map | Populated (architecture migration group). Domain-specific map only. |
...
| External references | `docs/governance/authority/EXTERNAL_REFERENCES_AUTHORITY_INDEX.md` | candidate support map | Populated (external-references migration group). Non-authoritative unless distilled. |
