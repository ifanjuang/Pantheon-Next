...
| Internal consequential-write chokepoint | partial — wired on one entry point of eight | The Cockpit's Knowledge write now resolves a policy decision point before the request body is examined. Enforcement defaults to `required`: an unconfigured decision point refuses the write with `503`, and a deployment that runs without one must declare `MVP_POLICY_ENFORCEMENT=disabled` by name. Read-only projections are unaffected. The other seven mutation entry points under `implementation/` remain held by module-local guards only. `implementation/tests/test_consequential_mutation_inventory.py` enumerates every entry point with its actual guard; `test_policy_client_assembly.py` covers the assembly so the wiring cannot be lost silently. A real decision point has now been observed end to end: `implementation/tools/observe_policy_round_trip.py` drives `HttpPolicyClient` into `enforce_consequential` against a running `pantheon-policy-api` and records both directions — refused without gate signals (`blocked_pending_task_contract`, naming the missing references), allowed with them (`eligible_with_gate_signals_unverified`). The allowing disposition is itself bounded: the decision point reports `gate_signal_validation_performed: false`, so those references remain caller-asserted. Remaining proof: the seven other mutation entry points reviewed one by one, and a decision point observed in a real deployment rather than a local process. Gate answered is not effect performed. |
