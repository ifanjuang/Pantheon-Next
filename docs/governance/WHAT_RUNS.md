...
|---|---|---|
| Hermes policy/PEP integration | co-located candidate / local policy round-trip observed / deployment enforcement not established | The real `HttpPolicyClient -> enforce_consequential -> pantheon-policy-api` path has been observed locally in both refusal and allow directions without performing an effect. Demonstrate the applicable decision point in a real selected deployment and validate the required gate references rather than merely receiving caller-asserted identifiers. |
| Internal consequential-write chokepoint | partial — one of eight reviewed entry points wired; 64 further mutation entry points enumerated but unreviewed | The #877 inventory net currently enumerates 72 mutation entry points. Eight have been individually reviewed: the Cockpit Knowledge UPDATE apply path now resolves a policy decision point before the request body is examined, while the other seven reviewed paths remain held by module-local guards. The other 64 entries are explicitly `unreviewed`; their guard regime is unknown rather than assumed. Enforcement on the wired Knowledge path defaults to `required`: an unconfigured decision point refuses the write with `503`, and a deployment that runs without one must declare `MVP_POLICY_ENFORCEMENT=disabled` by name. Read-only projections are unaffected. `implementation/tests/test_consequential_mutation_inventory.py` owns the enumerable surface and `test_policy_client_assembly.py` covers the application assembly so this wiring cannot disappear silently. `implementation/tools/observe_policy_round_trip.py` drives the real policy client against a running local `pantheon-policy-api`: it observes refusal without gate signals and an `eligible_with_gate_signals_unverified` disposition when caller-supplied gate references are present. The decision point itself reports that those references are not validated. Remaining proof: review the 64 enumerated entries, decide which are consequential application boundaries versus lower-level persistence helpers, extend central enforcement only where the governed effect boundary requires it, and observe the selected deployment. One wired path is not mutation-surface closure; gate answered is not effect performed. |
