...
| Common installation baseline | candidate support doctrine / documented non-implemented | `COMMON_INSTALLATION_BASELINE.md` selects one common component baseline instead of user-selected Pantheon presets. | It creates no installer, stack, database, binding or activation. Required presence does not authorize use. |
...
| Install module catalog | candidate only / documented non-implemented | `INSTALL_MODULE_CATALOG.md` defines a generic candidate grammar for module records, dependency roles, conflict classes, source trust, provisioning, gates, health, rollback and update policy. Its preset examples are not the active installation contract. | It does not create live catalog files, schemas, tests, registry, installer, plugin marketplace, Docker/Portainer stack, shell runner, provider router, approval engine or memory engine. |
