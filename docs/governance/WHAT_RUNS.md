...
| Pantheon MVP Vertical binding | documented non-implemented / external executable candidate | `PANTHEON_MVP_VERTICAL_BINDING.md` classifies the published `ifanjuang/pantheon-mvp` repository, initially reviewed as the `pantheon-mvp-vertical` bundle, as a candidate binding for `governed_task_loop_block_1`. | It is not imported, installed, executed, activated or adopted by Pantheon. Adoption remains blocked pending pushed repository content, CI observation and P0 fixes: Task Contract schema alignment, path-boundary checks, fixture-specific runner labelling and related tests. |
