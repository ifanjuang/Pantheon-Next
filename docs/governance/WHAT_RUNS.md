...
| Internal consequential-write chokepoint | partial — two reviewed application boundaries wired; wider surface remains open | The mutation inventory now enumerates 92 entry points, and all 92 have been individually reviewed; the unreviewed backlog is closed and its ceiling is 0 — a newly discovered entry point is reviewed when it is added, not admitted to a backlog. Two write paths are wired to `enforce_consequential`. The Cockpit Knowledge UPDATE apply route resolves the Pantheon decision client under a fail-closed default posture; an unconfigured decision point refuses that route, while local-guards-only operation must be selected explicitly. `human_access.bind_oidc_identity` — the write that makes an external OIDC identity able to act as a governed principal, and the root of trust every authenticated request resolves against — routes through the same chokepoint when a decision client is supplied, with the decision bound to a digest of the exact binding rather than to its name; its only production path, `mvp-vertical bind-oidc-identity`, fails closed without a configured decision point unless `MVP_POLICY_ENFORCEMENT=disabled` is declared explicitly. Both underlying primitives still accept an optional policy client for bounded direct/test use, so the inventory continues to describe that function-level fact rather than overclaiming universal enforcement. When a client is supplied, each owner re-binds the effect it validates from what it itself observed rather than from what a caller supplied, so a caller-supplied candidate cannot broaden it. Five reviewed entries are currently `gate_required_not_wired` (`store_reviewed_dossier`, `publish_knowledge`, `complete_edit_request`, `apply_edit_request`, `act_working_information`); that debt is tracked by the mutation inventory and must not grow silently, and each entry is now bound to the absence of the gate rather than to a code fragment that would survive its own repair. Remaining proof: wire the five remaining gates or record why each stays local-only, and observe the selected deployment. A validated decision is not the applied effect; the returned effect-binding trace is not Evidence, approval or production proof. |
