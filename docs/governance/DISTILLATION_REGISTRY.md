...
| Measurability coverage disclosure | iFixAi | expose which checks were runtime-observed, declared, synthesized, judge-assessed, exploratory or unmeasurable before presenting an aggregate result | `WATCHLIST.md`, future Hermes evaluation binding review | candidate |
| Declared governance demotion | iFixAi | treat governance supplied by fixture or synthesized from configuration as a declaration candidate, never as runtime verification, evidence sufficiency or approval | `EVIDENCE_PACK.md`, `EXTERNAL_TOOLS_POLICY.md`, future Hermes evaluation binding review | candidate |
