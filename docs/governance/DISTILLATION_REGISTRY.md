...
| Bounded evaluation / simulation lab | Future AGI | keep simulation and evaluation outside Hermes core behind one optional adapter; return candidates only and refuse the platform gateway, provider router, workflow engine, privileged executor and optimizer as runtime authority | `HERMES_CAPABILITY_BINDINGS.md`, `EXTERNAL_TOOL_PLACEMENT_REGISTER.md`, `PRE_EXECUTION_SIMULATION.md` | hermes_candidate_constraint |
...
| Versioned multi-repository context manifest | BlitzOS | carry an explicit repository map, relationships, conventions, scope and staleness as a tool-specific adapter generated from the canonical Context Pack | `CONTEXT_PACKS.md`, `HERMES_INTEGRATION.md`, `HERMES_CAPABILITY_BINDINGS.md` | candidate |
| Factual session handoff record | BlitzOS | preserve a concise work record with changed objects, verification, decisions, unresolved gaps and next action; keep it distinct from evidence, approval and canonical memory | `CONTEXT_PACKS.md`, `EVIDENCE_TOPOLOGY.md`, `templates/context_handoff/SESSION_HANDOFF.md` | active_support_pattern |
| Candidate-only context maintenance | BlitzOS | propose context updates as reviewable diffs or PRs; never auto-merge, write the default branch silently or promote session observations to memory | `CONTEXT_PACKS.md`, `MEMORY.md`, `APPROVALS.md`, `REJECTED_PATTERNS.md` | active_support_pattern |
