...
- `architecture-project-understanding/` (Project Anatomy V0.2-only validation family. The project-world core is `stable_object`, `source_representation`, `attribute_claim` and `relation_claim`; `observation_bundle` is the candidate-only adapter exchange envelope and does not add a fifth primitive or persistence owner; `program`, `requirement`, `classification_scheme`, `calibration`, `derivation` and `contradiction` are supporting contracts. Evidence, decisions and approvals retain their existing owners. See `docs/domain-packs/architecture/PROJECT_ANATOMY_MODEL.md`, `docs/domain-packs/architecture/PROJECT_ANATOMY_BASELINE_DECISION.md` and `docs/governance/PROGRAM_AND_CONFORMANCE.md`.)
