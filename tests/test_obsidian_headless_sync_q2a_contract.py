...
def test_runtime_is_manual_and_fail_closed_on_synthetic_inputs():
    ...
    for name in ("OBSIDIAN_SYNC_EMAIL", "OBSIDIAN_SYNC_PASSWORD", "OBSIDIAN_SYNC_VAULT"):
        assert f"secrets.{name}" in text
        assert f'test -n "${name}"' in text
    assert "obsidian-headless@0.0.14" in text


def test_q2a_has_real_cli_preflight_without_credentials():
    text = WORKFLOW.read_text(encoding="utf-8")
    assert "cli-preflight:" in text
    assert "needs: [contract, cli-preflight]" in text
    for command in (
        "login",
        "logout",
        "sync-list-remote",
    ...
    for flag in ("--device-name", "--json", "--mode", "--conflict-strategy"):
        assert flag in text
...
def test_q2a_treats_concurrent_edit_recoverability_as_safety_gate():
    text = RUNNER.read_text(encoding="utf-8")
    assert "Safety sentinel for upstream concurrent-edit handling" in text
    assert 'grep -R -q "device-a:$PANTHEON_Q2A_RUN_ID"' in text
    assert 'grep -R -q "device-b:$PANTHEON_Q2A_RUN_ID"' in text
    assert '"concurrent_conflict_preserves_both_markers": "pass"' in text


def test_q2a_observes_crud_continuous_and_restart_without_overclaiming():
    text = RUNNER.read_text(encoding="utf-8")
    for needle in (
